...
METAL_forestplot_update <- function(tbl,all,rsid,flag="",package="meta",method="REML",split=FALSE,outpath="",...)
{
  prot <- MarkerName <- NA
  ...
  for(i in 1:nrow(tbl))
  {
     ...
     TITLE <- sprintf("%s [%s (%s) (%s/%s) N=%.0f P=%G] %s",p,m,t[i,"rsid"],A1,A2,tbl[i,"N"],pv,f)
     ...
     with(sall, {
       ...
       if (package=="meta")
       {
         ...
         meta::forest(mg, prediction = TRUE,
                      colgap.forest.left = "1cm",
                      leftlabs=c("Study","b","SE"),...)
       ...
       if (split) dev.off()
       message("Saving forest plot to ", paste0(outpath,p,"-",o,".pdf"))
     })
  }
}
